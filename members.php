...
if(isset($_GET["userid"])){
	die(header("location:user-edit.php"));
}
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  ...
                  <p class="card-category"> Membres ayant accès au panel Cipher</p>
                </div>
                <div class="card-body">
                            ...
                            if($result = mysqli_query($db, $sql)){
                                if(mysqli_num_rows($result) > 0){
                                    while($row = mysqli_fetch_array($result)){ ?>
				                        <tr>
				                          <td>
				                            <img src="<? echo $row['img']; ?>" height=50 width=50 class="avatar-cipher">
				                          </td>
				                          <td>
				                             <? if ($row['ban'] == 0){
                                      echo $row['username']; }else{
                                        echo '<strike>'.$row['username'].'</strike>'; } ?>
				                          </td>
				                            <? if ($row['rank'] == 1 and $row['ban'] == 0) { ?>
				                              <td>
                                      Membre
                                    </td>
                                     <? }elseif($row['ban'] == 1) {
                                    ...
                                    if($resultc = mysqli_query($db, $check)){
                                      if(mysqli_num_rows($resultc) > 0){
                                        while($rowa = mysqli_fetch_array($resultc)){
                                          if ($rowa["rank"] == 2){ ?>
                                            <a href="user-edit.php?userid=<? echo $row["id"]; ?>">
        				                          	<button type="button" class="btn btn-warning"><i class="fas fa-pen"></i>&nbsp; Contrôle</button>
                                            </a>
                                      <? }
                                      }
                                    }
